...
const PENDING_PATH = path.join(__dirname, 'batch_pending_v3.json');
...
function run() {
    console.log('Applying Batch 3 results...');
    const tracker = JSON.parse(fs.readFileSync(TRACKER_PATH, 'utf8'));
    const pendingJson = JSON.parse(fs.readFileSync(PENDING_PATH, 'utf8'));
    
    // IDs from browser subagent report
    const salvageIds = ["126", "128", "129", "130", "133", "134", "136", "137", "139", "140", "141", "144", "147", "148", "149", "150", "151"];
    const manualIds = ["124", "125", "127", "138", "153", "154"];
    ...
    salvagedShops.forEach(shop => {
        if (!currentIds.has(shop.id)) {
            // Give it a temp address if missing (or use search result info if we had it - but ID is enough to pull from master)
