...
shops.forEach(s => {
    ...
    for (const rule of rules) {
        if (rule.nameRegex.test(s.name)) {
            // Check if mapped area strongly differs from rule
            let isBad = false;
            if (rule.invalidAreaRegex && rule.invalidAreaRegex.test(s.area)) {
                // If the shop name says Umeda, but it's mapped to namba... it's a mismatch
                // Except chains like "梅田屋" in namba? Usually not.
                isBad = true;
            }
