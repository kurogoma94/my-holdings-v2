...
const rules = [
    { code: 'udon', keywords: ['うどん', 'そば', '蕎麦', 'きすけ', '兎麦', 'いなの路', '三国', 'Kyutaro'] },
    { code: 'italian', keywords: ['イタリア', 'ピザ', 'ピッツェリア', 'Pizzeria', 'Dal', 'Trattoria', 'バル', 'ワイン', 'pasta', 'ぱすた', 'パスタ', 'PISOLA', 'カーサビアンカ', 'Macauda', 'La Lanterna', 'イルテラ'] },
    { code: 'yoshoku', keywords: ['洋食', 'グリル', 'ハンバーグ', 'オムライス', 'フレンチ', 'ビストロ', 'とんかつ', 'コロッケ', 'フランス', 'ステーキ', 'テキ', 'ル・プログレ', 'ヴァリエ', 'ル・コントワ', 'マルヨシ'] },
...
others.forEach(s => {
    ...
    // Some hardcodes for obvious ones based on names
    if (name.includes('サロン卵と私')) matched = 'yoshoku';
    
    counts[matched] = (counts[matched] || 0) + 1;
});

console.log("Counts per proposed new genre:");
